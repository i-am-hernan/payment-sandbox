...
},
{
    ...
    type: 'switch',
    configObject: {}, // Remove allowPaymentMethods when enabled
...
const applyFeatureToConfig = (
    ...
    // Special case for local payment methods
    if (feature.id === 'local-payment-methods') {
        if (enabled) {
            // When enabled, remove allowPaymentMethods
            delete newConfig.allowPaymentMethods;
        } else {
            // When disabled, add allowPaymentMethods: ["scheme"]
            newConfig.allowPaymentMethods = ["scheme"];
        }
        return newConfig;
    }
